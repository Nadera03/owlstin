  ...
  return (
    <header 
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled 
          ? "bg-magical-midnight/90 backdrop-blur-md py-3 shadow-lg" 
          : "bg-transparent py-5"
      }`}
    >
      ...
      {/* Mobile Menu */}
      {isMenuOpen && (
        <div className="md:hidden fixed inset-0 top-[60px] bg-magical-midnight/95 backdrop-blur-md z-40 flex flex-col">
...
const MobileNavLink = ({ to, children, onClick }: { to: string; children: React.ReactNode; onClick: () => void }) => (
  <Link 
    to={to}
    className="text-magical-starlight text-2xl font-cinzel hover:text-magical-glowing-teal transition-colors duration-300"
    onClick={onClick}
  >
    {children}
  </Link>
